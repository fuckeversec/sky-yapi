...
public class BuildJsonForYApi {
    ...
    public ArrayList<YapiApiDTO> actionPerformedList(AnActionEvent anActionEvent, String attachUpload) {
        ...
        String classMenu = DesUtil.getDesc(selectedClass);
        ...
        if (Objects.equals(selectedClass.getName(), selectedText) || Objects.isNull(selectedText)) {
            ...
            return processingSelectedClass(attachUpload, project, selectedClass, classMenu, yApiApiDTOs);
        } else {
            // 选择的是方法 则处理此方法
     ...
     */
    private ArrayList<YapiApiDTO> processingSelectedMethod(String attachUpload, String selectedText, Project project,
            PsiClass selectedClass, String classMenu, ArrayList<YapiApiDTO> yapiApiDTOS) {
        PsiMethod[] psiMethods = selectedClass.getAllMethods();
        //寻找目标Method
        PsiMethod psiMethodTarget = Stream.of(psiMethods).filter(psiMethod -> psiMethod.getName().equals(selectedText))
        ...
        if (Objects.nonNull(psiMethodTarget)) {
            ...
            Notification error = notificationGroup
                    .createNotification("can not find method:" + selectedText, NotificationType.ERROR);
     ...
     * @return the array list
     */
    private ArrayList<YapiApiDTO> processingSelectedClass(String attachUpload, Project project, PsiClass selectedClass,
            String classMenu, ArrayList<YapiApiDTO> yapiApiDTOS) {
     ...
     */
    protected YapiApiDTO actionPerformed(PsiClass selectedClass, PsiMethod psiMethodTarget, Project project,
            String attachUpload) {
        ...
        if (psiAnnotationMethod != null) {
            ...
            if (Objects.nonNull(requestMethodUtil.getPsiAnnotationMethodSemple())) {
                PsiNameValuePair[] psiNameValuePairs = requestMethodUtil.getPsiAnnotationMethodSemple()
        ...
        try {
            ...
            if (!Strings.isNullOrEmpty(attachUpload)) {
                ...
                if (codeSet.size() > 0) {
                    ...
                    if (!Strings.isNullOrEmpty(attachUpload)) {
                        String fileUrl = new UploadYapi()
                                .uploadFile(attachUpload, project.getBasePath() + codeFileName);
                    ...
                    if (psiAnnotation != null) {
                        ...
                        if (psiNameValuePairs.length > 0) {
                            for (PsiNameValuePair psiNameValuePair : psiNameValuePairs) {
                                if ("name".equals(psiNameValuePair.getName()) || "value"
                                    ...
                                    if (yapiHeaderDTO != null) {
                                        ...
                                        yapiPathVariableDTO
                                                .setName(psiNameValuePair.getValue().getText().replace("\"", ""));
                                    } else {
                                    ...
                                    yapiQueryDTO.setRequired(psiNameValuePair.getValue().getText().replace("\"", "")
                                            .replace("false", "0").replace("true", "1"));
                                } else if ("defaultValue".equals(psiNameValuePair.getName())) {
                                    if (yapiHeaderDTO != null) {
                                        yapiHeaderDTO
                                                .setExample(psiNameValuePair.getValue().getText().replace("\"", ""));
                                    } else {
                                        yapiQueryDTO
                                                .setExample(psiNameValuePair.getValue().getText().replace("\"", ""));
                                    }
                                    ...
                                    if (yapiHeaderDTO != null) {
                                        yapiHeaderDTO.setName(StringUtils.isNotBlank(psiNameValuePair.getLiteralValue())
                                                ? psiNameValuePair.getLiteralValue() : psiParameter.getName());
                                        // 通过方法注释获得 描述 加上 类型
                                    ...
                                    if (yapiPathVariableDTO != null) {
                                        yapiPathVariableDTO.setName(
                                                StringUtils.isNotBlank(psiNameValuePair.getLiteralValue())
                                                        ? psiNameValuePair.getLiteralValue() : psiParameter.getName());
                                        // 通过方法注释获得 描述 加上 类型
                                        ...
                                        yapiQueryDTO.setName(StringUtils.isNotBlank(psiNameValuePair.getLiteralValue())
                                                ? psiNameValuePair.getLiteralValue() : psiParameter.getName());
                                        // 通过方法注释获得 描述 加上 类型
                                        yapiQueryDTO.setDesc(
                                                DesUtil.getParamDesc(psiMethodTarget, psiParameter.getName()) + "("
                                                        + psiParameter.getType().getPresentableText() + ")");
                                    }
                            ...
                            if (yapiHeaderDTO != null) {
                                ...
                                yapiQueryDTO.setName(psiParameter.getName());
                                // 通过方法注释获得 描述 加上 类型
                                yapiQueryDTO.setDesc(DesUtil.getDesc(project, psiParameter.getType()));
                            }
                        ...
                        if (yapiApiDTO.getReqBodyForm() != null) {
                            ...
                            yapiApiDTO.setReqBodyForm(getRequestForm(project, psiParameter, psiMethodTarget));
                        }
                    }
                }
            }
    ...
    public static List<Map<String, String>> getRequestForm(Project project, PsiParameter psiParameter,
        ...
        if (NormalTypes.NORMAL_TYPES.containsKey(psiParameter.getType().getPresentableText())) {
            ...
            for (PsiField field : psiClass.getAllFields()) {
                ...
                map.put("type", "text");
                String remark = DesUtil.getLinkOrSeeRemark(project, field);
                map.put("desc", remark);
        ...
        if (psiType instanceof PsiPrimitiveType) {
            ...
            if (types.length > 1) {
                ...
                if (psiClassChild.isEnum()) {
                    result.set("description", DesUtil.getEnumDesc(psiClassChild));
                } else {
                    result.set("description", (psiType.getPresentableText() + " :" + psiClassChild.getName()).trim());
                }
                result.set("properties", kvObject);
    ...
    public static void getField(PsiField field, Project project, KV kv, String[] childType, Integer index,
        ...
        if (type instanceof PsiPrimitiveType) {
            ...
            if (NormalTypes.isNormalType(fieldTypeName)) {
                ...
                ObjectNode objectNode = JsonUtil.OBJECT_MAPPER.createObjectNode();
                objectNode.put("type", DesUtil.enumType(project, field));
                if (!Strings.isNullOrEmpty(remark)) {
                    objectNode.put("description", remark);
                }
                kv.set(name, objectNode);
                ...
                if (!pName.equals(((PsiClassReferenceType) type).getClassName())) {
                    ...
                    List<String> requiredList = new ArrayList<>();
                    kv1.set(KV.by("properties",
                            getFields(PsiUtil.resolveClassInType(type), project, childType, index, requiredList)));
                    kv1.set("required", requiredList);
                } else {
        ...
        Set<String> changeFilePaths = filePaths.stream().map(filePath -> {
            if (filePath.contains(".jar")) {
                ...
                try {
                    ...
                    Notification error = notificationGroup
                            .createNotification("can not find sources java:" + jarPath, NotificationType.ERROR);
                    Notifications.Bus.notify(error, project);
                }
            }
            return filePath;
